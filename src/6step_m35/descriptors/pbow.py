...
class PBOW(object):
    """
    1. Divide an input image into increasingly fine, granular sub-regions.
    2. Extract BOVW histograms from each sub-region.
    3. Weight the histograms according to the current pyramid level.
    4. And finally, return the concatenated feature histograms as the final feature vector.
    ...
    def describe(self, imageWidth, imageHeight, kps, features):
        ...
        for level in np.arange(self.numLevels, -1, -1):
            # compute the number of partitions and the weight for the current level
            numParts = 2 ** level

            # Lazebnik et al. equation.
            ...
            # import itertools
            # itertools.product(np.arange(len(xPart), np.arange(len(yParts)))
    ...
    def featureDim(numClusters, numLevels):
        # compute and return the resulting dimensionality of the PBOW feature representation
